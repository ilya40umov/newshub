 ...
 * @author ilya40umov
 */
class FeedRepositorySpec extends RepositorySpec[FeedRepository] {

  implicit val ctx = scala.concurrent.ExecutionContext.global

  override def newRepository(client: ElasticClient): FeedRepository = new FeedRepositoryImpl(client)
